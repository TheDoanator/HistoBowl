      ...
      <div className='min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 transition-[background-color] duration-300 ease-out flex flex-col pb-12'>
        ...
        <nav className="border-b bg-white/80 dark:bg-slate-900/80 border-slate-200 dark:border-slate-800 backdrop-blur-sm transition-[background-color,border-color] duration-300 ease-out">
          <div className="max-w-[90%] xl:max-w-[85%] mx-auto px-2 sm:px-4 flex justify-between h-16 items-center">
            ...
            {/* Mobile: theme toggle stays reachable, hamburger opens the nav panel */}
          ...
          {isMobileMenuOpen && (
            <div className="md:hidden border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-4 py-3 flex flex-col gap-1">
              <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-black italic tracking-wide uppercase hover:text-orange-600 py-2">
                HOME
              </Link>
              <Link to="/tournaments" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-black italic tracking-wide uppercase hover:text-orange-600 py-2">
                TOURNAMENTS
              </Link>
            </div>
          )}
...
function BroadcastTicker() {
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 bg-slate-900 dark:bg-black border-t border-slate-800 h-12 flex items-center overflow-hidden z-50 shadow-2xl">
      <div className="bg-orange-600 text-white px-4 sm:px-6 h-full flex items-center font-black text-xs uppercase tracking-widest shrink-0 z-10 shadow-lg">Updates</div>
